...
class CanvasStudentMCP:
                ...
                Tool(
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            "username": {
                                "type": "string",
                                "description": "Student username/email",
                            },
                            "password": {
                                "type": "string",
        ...
        async def call_tool(
            ...
            try:
                if name == "authenticate_student":
                    ...
                    return await self._get_module_content(
                        arguments["course_id"], arguments["module_id"]
                    )
                    ...
                    return await self._search_course_content(
                        arguments["course_id"], arguments["query"]
                    )
    ...
    async def _authenticate_student(
    ...
    async def _get_student_courses(self) -> Sequence[TextContent]:
        ...
        if not self._check_authentication():
            return [
                TextContent(
                    type="text",
                    text="❌ Please authenticate first using authenticate_student",
                )
            ]
    ...
    async def _get_module_content(
        ...
        try:
            ...
            content_list = "\n".join(
                [f"📄 {item['title']} ({item['type']})" for item in items]
            )
            ...
            return [
                TextContent(
                    type="text", text=f"Error fetching module content: {str(e)}"
                )
            ]
        ...
        try:
            ...
            assignment_list = "\n".join(
                [
                    ...
                    f"({assignment.get('points_possible', 'N/A')} pts)"
            ...
            return [
                TextContent(type="text", text=f"Error fetching assignments: {str(e)}")
            ]

    async def _search_course_content(
        self, course_id: int, query: str
    ) -> Sequence[TextContent]:
        ...
        try:
            ...
            return [
                TextContent(
                    ...
                    f"{result.get('documents_indexed', 0)} documents for course {course_id}",
                )
            ]
    ...
    def _check_authentication(self) -> bool:
        ...
        return self.canvas_client is not None and self.session_data.get(
            "authenticated", False
        )
